...
\label{defJ1}
\end{equation}
where integral $J_4^*$ and $J_5^*$ are defined by equations \eqref{defJ4} and \eqref{defJ5} and their expressions are given \eqref{valJ4} and \eqref{valJ5} respectively.
...
\label{decompJ2}
\end{equation}
where $P(t)=b(1-t^2)-2\nuti it\cos\tilde{\varphi}+ \sin \tilde{\varphi} \nuti(1+t^2)$ and $Q(t)=2\nuti t-ia(1-t^2)$ and the coefficients of the decomposition are determined in the sequel.
...
\label{nsurpq}
\end{equation}
The roots of polynomial $P$ are $p_\pm$ and have been defined by equation (\ref{p1p2}). The roots of polynomial $Q$ are $q_\pm$ and are defined by equation (\ref{q1q2}). Multiplying \eqref{nsurpq} by $P(t)$ and evaluating the result at $t=p_+$ and $t=p_-$ leads to the following system
\begin{eqnarray}
\left\{
\begin{array}{l}
...
\label{valJ2}
\end{equation}
with $A\rightarrow +\infty$ and $t_{\nuti}\rightarrow 1$. These diverging terms will be compensated in the final computation of matrix $\mathbb{T}$, as seen in section \ref{calcI3}.
...
\end{equation}
The coefficients of the decomposition are obtained using \eqref{nsurpq}, \eqref{etapsi} and \eqref{lambdarho}. Finally :
\begin{equation}
\begin{split}
...
\label{decompJ4}
\end{equation}
The first step is to determine $\gamma_4$ by multiplying the above decomposition by $(1-t)^2$ and evaluating the result at $t=1$. Similarly, $\delta_4$ is determined by multiplying \eqref{decompJ4} by $(1+t)^2$ and evaluating the result at $t=-1$ :
\begin{subequations}
\begin{equation}
...
\end{split}
\end{equation}
The final coefficients are deduced using \eqref{nsurpq}, (\ref{etapsi}) and (\ref{lambdarho}). Finally
\begin{equation}
\begin{split}
...
\end{equation}
Coefficients $\eta_5,\psi_5,\lambda_5,\rho_5$ are determined using \eqref{nsurpq}, \eqref{etapsi} and \eqref{lambdarho}. Finally
\begin{equation}
\begin{split}
...
\end{split}
\end{equation}
The final coefficients are determined using \eqref{nsurpq}, (\ref{etapsi}) and (\ref{lambdarho}). Finally
\begin{equation}
\begin{split}
J_6= & 2\sin\phiti\ln\left(\frac{1+t_{\nuti}}{1-t_{\nuti}}\right)\\
...
\begin{equation}
\begin{split}
J_7&= 2i\cos\phiti\log\left(\frac{1+t_{\nuti}}{1-t_{\nuti}}\right) \\
...
We begin by computing terms derived from $\mathbf{tm}_L$, defined by \eqref{tmL}.
\begin{multline}
...
Let us now compute terms derived from $\mathbf{tm}_T$, defined by \eqref{tmT}.
\begin{multline}
...
This concludes computation of coefficients of matrix $\mathbb{T}_{kl}$ in the 2D elastic case. It has been verified that all diverging terms $\log\left(\dfrac{1+t_{\nuti}}{1-t_{\nuti}}\right)$ that appear in integrals $J_1^*$ to $J_5^*$ compensate each other when the L and T terms are summed, as explained in \ref{calcI3}.
