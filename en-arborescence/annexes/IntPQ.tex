...
\label{rogsog}
Let us introduce ${\rm rog}(a)$ and ${\rm sog}(a)$ complex functions defined for $a > 1$ as
\begin{equation}
\label{defrog}
...
\end{equation}
These functions are used in the sequel to give an exact expression of the coefficients of matrices $\mathbb{D}(a,b)$ and $\mathbb{T}(a,b)$. Their analytic properties are given hereafter.
...
{\bf Proof}
The roots of the polynomial $a(1-x^2)+2ix, a\in \mathbb C^* $ are $x_\pm={1\over a}(i\pm \sqrt {a^2-1})$. When $a \notin \{ -1,1\} $, these roots are distinct and  formula \eqref{rog_function} results from a classical integral computation. However, when $a=1$, \eqref{defrog} yields rog$(1)=1$ whereas \eqref{rog_function} presents a singularity. The indetermination is lifted by defining $z=\sqrt{1-1/a^2}$ in \eqref{rog_function}. We then have $a=\frac{1}{\sqrt{1-z^2}}$ (note that the variable change is therefore valid for $a=1$ and not for $a=-1$) and :
\begin{equation}
...
\end{equation}
where the overline here denotes the complex conjugate. Equation \eqref{intQ} can now be applied (using the definition of $Q(t)$ given in \eqref{B:defQ}) :
\begin{equation}
