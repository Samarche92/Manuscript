...
and the Dirichlet \eqref{Chapter5:CLDir} or Neumann \eqref{Chapter5:CLNeu} boundary condition on the wedges faces
...
\frac{\partial g}{\partial n_j} \mid_{F_j} = 0, \quad j=1,2.
\end{equation}
\end{subequations}
...
Similarly, the system \eqref{Chapter5:WaveMotion}-\eqref{Chapter5:CLNeu} is equivalent to the following  system of equations for the dimensionless problem
\begin{equation}
\label{Chapter5:Adimen_waveMotion_Neu}
\begin{cases}
(\triangle+1)v =  0 \quad \text{in } \Omega_f, \\
\frac{\partial v}{\partial n_j} =  -\frac{\partial h_{\rm inc}}{\partial n_j}  \quad \quad \quad \text{on } F_j, \quad j=1,2
\end{cases}.
\end{equation}

In order to obtain a solution to this problem which is physically relevant, the limiting absorption principle is used. It consists in substituting the wave number $k_0$ by a complex one $k_0 e^{-i\epsilon} $ with $\epsilon > 0$. This means that absorption occurs in the medium and thus the scattered waves attenuate with the distance. The system (\ref{Chapter5:Adimen_waveMotion_Dir}) then becomes :
\begin{equation}
\label{Chapter5:waveMotion_epsilon_Dir}
(S_\epsilon^*) \quad
\begin{cases}
...
\end{cases}
\end{equation}
and system (\ref{Chapter5:Adimen_waveMotion_Neu}) becomes :
\begin{equation}
\label{Chapter5:waveMotion_epsilon_Neu}
(S_\epsilon^*) \quad
\begin{cases}
(\triangle+ \, e^{-2i\epsilon}) v^\epsilon  =  0 \quad \text{in } \Omega_f, \\
\frac{\partial v}{\partial n_j} =  -\frac{\partial h_{\rm inc}}{\partial n_j}  \quad \quad \quad \text{on } F_j, \quad j=1,2
...
The physically relevant solution to (\ref{Chapter5:Adimen_waveMotion_Dir}) or \eqref{Chapter5:Adimen_waveMotion_Neu}, called the outgoing solution, can now be defined. It is the one obtained when taking $\epsilon \to 0$ in (\ref{Chapter5:waveMotion_epsilon_Dir}) or \eqref{Chapter5:waveMotion_epsilon_Neu} respectively. This limit is noted $v^0$. Its integral representation is found hereafter.
...
-- (3,3.62)
-- cycle;
\fill[color=white] (4,0) circle (0.1);
...
\node at (1.1,-1.3) {$-\cos \widetilde{2\varphi}$};

%  Omega_0
\draw [thick, ->] (3.5,3)--(2.5,3);
\node at (4,3) {$\mathbf{\Omega_0}$};
