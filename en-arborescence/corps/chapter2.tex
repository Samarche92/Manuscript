...
Once again, the residues $V_j^{(1)}$ of these generated poles $Z_j^{(1)}$ are determined by the fact that they should cancel the singular term in $DM(X_j')(\xi)$, found using formula \eqref{Int_op_decomp_DM}, compensating the singular term in the $TM$ operator in \eqref{Chapter5:pol_propag_step3}.
...
\begin{center}
	...
	\draw[dashed, ->,>=stealth](0,-3.5)--(0,3) node[left]{$\tau$};
