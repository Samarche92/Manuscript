\chapter{Acoustic wedge with mixed boundary conditions}
...
For a certain time, methods of computation have been studied without proof of solvability for the wedge diffraction problem. Castro and Kapanadze \cite{Castro} have proven existence and uniqueness of the solution for acoustic and electromagnetic waves using a detailed Fredholm analysis. Kamotski and Lebeau \cite{KamotskiLebeau} have proven existence and uniqueness of the solution to the elastic wave diffraction  by a soft wedge (Dirichlet boundary) problem for all angles using the Spectral Functions method in which the diffracted wave is modeled thanks to these spectral functions. This method was at first developed by Croisille and Lebeau \cite{CroisilleLebeau} who proposed a numerical algorithm in order to compute these functions for elastic wedges of angle lower than $\pi$ immersed in a fluid.

The aim of this paper is to develop and implement the methodology of Croisille and Lebeau \cite{CroisilleLebeau} in the two-dimensional (i.e. the incident wave vector lies in the plane normal to the edge) case of an acoustic wave diffracted by a soft wedge immersed in a fluid (Dirichlet boundary condition) and propose a numerical validation of the method for angles both smaller and larger than $\pi$. This will be achieved by comparing the GTD approximation of the diffraction coefficient obtained using the spectral functions method, with the analytical expression given in \cite{Bouche,Bo} of the GTD approximation of the exact solution. 
...
\partial_t^2 g - c_0^2 \, \triangle g = 0
\end{equation}
...
The system \eqref{Chapter5:WaveMotion}-\eqref{Chapter5:CL} is equivalent to the following  system of equations for the dimensionless problem
\begin{equation}
...
\end{equation}
with $\alpha_j \in \mathcal{A}$ and where $\delta_{F_1}$ and $\delta_{F_2}$ are Dirac delta functions on the faces $F_1$ and $F_2$ of the wedge respectively.
...
The double Fourier transform of (\ref{inv_potentiels}) gives
\begin{equation}
\label{Fourier}
...
To compute the spectral functions, the functional equations satisfied by spectral functions $\hat{\alpha}_1$ and $\hat{\alpha}_2$ first has to be determined.
...
This function $T_0$ in \eqref{Trans_operator} operates in the domain $\Omega_0$ defined as
\begin{equation}
\label{Domain_Omega0}
...
It is well known that poles of the spectral functions lead to incident and multiply reflected fields on the wedge faces. The singular part of the spectral functions contains these poles. The goal of this subsection is to calculate the poles and the corresponding residues and then to determine the expression of the singular part of the spectral functions. The initial poles correspond to the incident field on the face wedges, and lead to the poles corresponding to the multiply reflected waves on the wedge faces, by employing a recursive algorithm. 
...
\begin{figure}[ht]%
\begin{center}
  \includegraphics[width=0.65\textwidth]{Figure6.pdf}
\end{center}
\caption
[Contour $\Gamma_2$]
...
\end{equation}
where $\hat{\alpha}_1$ and $\hat{\alpha}_2$ are the spectral functions, is compared to the analytic expression of the diffraction coefficient of the scattering of a plane wave with a wedge at interfaces fluid/void as expressed by Sommerfeld \cite{Sommerfeld}. Sommerfeld \cite{Sommerfeld} gives an expression of the coefficient in the case of the diffraction of a scalar plane wave by a wedge which can be used in the case of a stress-free wedge immersed in a fluid :
\begin{align}
\label{GTDCoeff_Som}
D^{\rm (Som)}(\theta) = \dfrac{e^{i\frac{\pi}{4}}}{2n \sqrt{2\pi}}  \, \left[ \cot \left( \dfrac{\pi + (\theta + \theta_{\rm inc})}{2n} \right) + \cot \left( \dfrac{\pi - (\theta + \theta_{\rm inc})}{2n} \right) \right.   \nonumber \\
...
\begin{subfigure}[b]{0.49\textwidth}
        \includegraphics[width=\textwidth]{Figure8a.pdf}
        \caption{$2\varphi = 80^o$, $\theta_{\rm inc} = 55^o$}
        \label{chapter5:figure12a}
    \end{subfigure}
\begin{subfigure}[b]{0.49\textwidth}
        \includegraphics[width=\textwidth]{Figure8b.pdf}
...
~\\
\begin{subfigure}[b]{0.49\textwidth}
        \includegraphics[width=\textwidth]{Figure8c.pdf}
        \caption{$2\varphi = 280^o$, $\theta_{\rm inc} = 240^o$}
        \label{chapter5:figure12d}
    \end{subfigure}
\begin{subfigure}[b]{0.49\textwidth}
        \includegraphics[width=\textwidth]{Figure8d.pdf}
        \caption{$2\varphi = 300^o$, $\theta_{\rm inc} = 150^o$}
        \label{chapter5:figure12c}
    \end{subfigure}
\caption
[Diffraction  coefficient computed with the recursive formula of spectral functions and with the Sommerfeld method]
{Diffraction  coefficient computed with the spectral functions and with the Sommerfeld method.}
...
On Figs.~\ref{chapter5:figure12a} and ~\ref{chapter5:figure12b} the wedge angles are lower than $\pi$ and on figs.~\ref{chapter5:figure12c} and \ref{chapter5:figure12d} the wedge angles are greater than $\pi$. In both cases, it appears clearly that both the regular part of the solution and the recursive method are necessary to obtain optimal results. When both of these are included, diffraction coefficients obtained with Spectral functions are close to those of the Sommerfeld method.


\section{Conclusion}
The spectral functions method is shown here to model diffraction of an acoustic wave from stress-free wedges. The diffraction coefficient obtained using the spectral functions has been compared to the analytic one obtained from the asymptotic evaluation of the Sommerfeld integral. The numerical results obtained thanks to the spectral functions method are very close to those given by the analytical solution and a precise result is obtained at a very low computational cost. The spectral functions method may therefore be extended to more complex wedge diffraction cases such as the computation of the diffraction coefficient of an elastic wave, wedges with different boundary conditions or in cases where the incident ray does not lie in the plane normal to the edge. In all these cases, studies may be carried out to numerically calculate corresponding diffraction coefficients for all wedge angles with a good precision and a low computation time.
