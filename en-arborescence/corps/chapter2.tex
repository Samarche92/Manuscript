...
\end{split}
\end{equation}
where $dm(\lambda) = \dfrac{1}{\zeta_0^0(\lambda)}$ in the case of Dirichlet boundary conditions and $dm(\lambda)=1$ in the case of Neumann boundary conditions. 
...
\end{split}
\end{equation}
where $tm(\lambda)=\dfrac{1}{\zeta_0^0(\lambda)}$ in the case of Dirichlet boundary conditions and $tm(\lambda)=\dfrac{\lambda\sin\varphi-t\cos\varphi\zeta_0^0(\lambda)}{\zeta_0^0(\lambda)}$ in the case of Neumann boundary conditions.
...
TM(\phi)(\xi) &= \int_{\Gamma_0} TM(\xi,\lambda) \cdot \dfrac{1}{\lambda - z} \, d \lambda = \dfrac{tm(z) }{\xi - T_0(z)} \mathbf{1} (z \in \Omega_0) + T_p(\xi,z),
\end{align}
\end{subequations}
...
TM  \left(  \dfrac{V_j}{\xi - Z_j} \right)(\xi) = \dfrac{tm(Z_j) \cdot V_j}{\xi - T_0(Z_j)}  \, \mathbf{1}(Z_j \in \Omega_0) + T_p(\xi,Z_j)\cdot V_j \qquad j=1,2
\end{equation}
...
Z_j^{(i+1)} = T_0(Z_{3-j}^{(i)}), \quad j,k \in \{1,2\}, \quad j=1,2
\end{equation} 
are the different generated poles with their respective residues 
\begin{equation}
\label{Generated_residues}
V_j^{(i+1)}=-dm^{-1}(T_0(Z_{3-j}^{(i)})) \,  tm(Z_{3-j}^{(i)})) \, V_{3-j}^{(i)} \, \mathbf{1}(Z_{3-j}^{(i)} \in \Omega_0), \quad  j=1,2.
\end{equation}
\paragraph{}
