...
\textcolor{red}{Snell's Law+fix (2.9) exp}
...
\end{equation}
The far field parameter is $R=\frac{\omega r}{c_L}$. The aim is to determine the asymptotic behaviour of $v(R\cos\theta\cos\delta,R\sin\theta\cos\delta)$ when $R\rightarrow +\infty$.The first step is to apply the change following of variables in integral $v_1$ :
\begin{equation}
\begin{split}
\xi&=\tilde{\nu}_*\cos\lambda \\
...
Note that in the case $\nuti_L =i\eta_L \in \mathbb{R}$, variable change \eqref{C4:changevar2}
...
\begin{subfigure}[b]{0.45\textwidth}
    	\begin{tikzpicture}
	\draw[step=1.5cm,gray,very thin,dashed](-1,-2.7)grid(3.7,3.7);
	\draw[thin] (-1,0)  -- (3.7,0) node[above]{$\lambda_1$};
	\draw[thin](0,-2.5)--(0,3.5) node[left]{$\lambda_2$};

	\node at (2.8,-0.2) {$\pi$};
	\node at (-0.2,0.2) {$0$};
	\node at (0.92,0.5) {$\lambda_s$};
	\node at (3,3.1) {$\mathcal{C}_0^T$};
	\node at (1.9,3.1) {$\gamma_{\theta}^T$};
...
The far-field evaluation of the integral is obtained by applying the steepest descent method, presented in appendix \ref{stationaryphase}. To do so, contour $C_0$ is deformed into contour $\gamma_{\theta}$, also visible in Fig.~\ref{C4:steepestcontour}. This leads to
\begin{equation}
v_1=v_1^{sing}+v_1^{diff}
\end{equation}
where $v_1^{sing}$ is the contribution of allthe singularities of the spectral functions crossed during the deformation from $C_0$ to $\gamma_{\theta}$, corresponding to the reflected and head waves, and $v_1^{diff}$ is the contribution of the stationary pahse point, corresponding to the reflected wave and computed using \eqref{steepformula}. Only the contribution of the diffracted waves will be computed here. In order to simplify notations, we will note $\mathbf{P}_*(\lambda,1)=\mathbf{P}_*(\lambda)$ and $\mathbf{P}_*(\lambda,-1)=\mathbf{P}_*(-\lambda)$). The contribution of diffracted waves is 
\begin{equation}
...
The diffraction coefficient is defined by
\begin{equation}
...
\begin{equation}
D_{\beta}^{\alpha}(\theta,\delta)=\frac{e^{-i\pi/4}}{2\sqrt{2\pi}}\sum_{j=1,2}\sum_{*=L,T}\nuti_*^2 \,{}^t \Sigma_j(-\nuti_*\cos\theta_j)\cdot\left(\mathbf{P}_*(\pi-\theta_j).\hat{i}_{\beta}\right)
\end{equation}
