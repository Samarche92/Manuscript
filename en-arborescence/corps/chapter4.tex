...
\end{equation}
$\cos\theta$ is well defined for $0\leq\rm Re \theta\leq \pi$, therefore this operator is well defined on subspace $\Omega_*^-$, visible on Fig.~\ref{C4:dOmegamoins} and defined as 
\begin{equation}
...
\begin{subfigure}[b]{0.45\textwidth}
	...
	\fill [color=gray!20]
	(-3,{-sinh(1.7)})
	-- plot [domain= -1.7:0] ({cosh(\x)},{sinh(\x)})
	-- (-3,0);
...
\node at (-2,-1.5) {$\Omega_*^-$};
	...
	\fill [color=gray!20]
	({-sinh(2)},-2)
	-- plot [domain= -2:2] ({sinh(\x)},0)
	--({sinh(2)},-2);
	
	%\fill[color=white] (-2,0) circle (0.25);
	
	%\fill[color=gray!20] (2,0) circle (0.25);
