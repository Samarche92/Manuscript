...
Commençons tout d'abord par appliquer une double transformée de Fourier à l'équation (\ref{vjdef}). Cela est licite car toutes les distributions qui interviennent dans cette équation sont tempérées et admettent donc une transformée de Fourier. On a alors :
\begin{equation}
\hat{v}^{\epsilon}_j(\xi,\eta)=(\mathbf{M}-e^{-2i\epsilon}\mathbf{\mathbb{I}_3})^{-1}\Sigma_j(\xi)
\label{matMvjeps}
\end{equation}
où les $\Sigma_j$  sont des fonctions inconnues appelées fonctions spectrales, définies par :
$$ \Sigma_j(\xi,)=\begin{pmatrix}
\hat{\alpha_j}(\xi)\\ \hat{\beta_j}(\xi) \\ \hat{\gamma_j}(\xi)
\end{pmatrix}$$

Et où \textbf{M} est la transformée de Fourier de l'opérateur $E$. En utilisant le fait que $\lambda+2\mu=1$, elle s'écrit : 
\begin{equation}
\mathbf{M}(\xi,\eta)=
\begin{pmatrix}
...
En utilisant (\ref{matM}) et le fait que $\mu=\frac{1}{\nu_T^2} $, on obtient :
\begin{multline}
...
% \begin{equation}
% a(\xi,\eta,\tau)=\frac{\xi^2+\nu_T^2(\eta^2+\tau^2)-\nu_T^2e^{-2i\epsilon}}{(\xi^2+\eta^2+\tau^2-e^{-2i\epsilon})(\xi^2+\eta^2+\tau^2-\nu_T^2e^{-2i\epsilon})}
% \end{equation}
% \begin{equation}
% b(\xi,\eta,\tau)=\frac{(1-\nu_T^2)\xi \eta}{(\xi^2+\eta^2+\tau^2-e^{-2i\epsilon})(\xi^2+\eta^2+\tau^2-\nu_T^2e^{-2i\epsilon})}
% \end{equation}

\paragraph{}

La formulation intégrale de chaque $v_j$ s'obtient finalement en inversant la double transformée de Fourier utilisée en (\ref{matMvjeps}) :
