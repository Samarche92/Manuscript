...
\end{tikzpicture}
\caption{Intermediate step in the deformation of contour $\Gamma_0$ into contour $\Gamma_1$.}
\end{subfigure}
~
\begin{subfigure}[b]{0.45\textwidth}
\begin{tikzpicture}[scale=0.8]
...
\end{tikzpicture}
\caption{Contour $\Gamma_1=\Gamma_1^a \cup \Gamma_1^b$.}
\label{C4:gamma1cr}
\end{subfigure}
\caption{Deformation of contour $\Gamma_0$ onto contour $\Gamma_1$ in the case $\cr$.}
\end{figure}
...
\node at (-1.1,1.5) {$\mathbf{\partial \Omega_T}$};
\node at (0.05,-0.35) {$-\nuti_T\cos \varphi$};
...
\end{tikzpicture}
\caption{Contour $\partial \Omega_T$. La flèche $\mathcal{F}_2$ montre la déformation du contour $\Gamma_0$ en $\partial \Omega_T$.}
\label{C4:domegaT}     
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.45\textwidth}
...
\end{tikzpicture}
\caption{Contour $\partial \Omega_L$.}
\label{C4:domegaL}
\end{subfigure}
\caption{Domaines de définition des opérateurs de translation}
\label{C4:domega0}
\end{figure}

In the case where $\noncr$, \eqref{C4:GaussDM} and \eqref{C4:GaussTM} are valid for $V \in \mathbb{C}^3, \, z\in \mathbb{C} \backslash  \rbrack - \infty, -\nuti_L \rbrack , \,  z \notin \{\tilde{\nu}_L,\tilde{\nu}_T \}, \, \mbox{Im} z \geq 0, \, \xi \in \Omega_*^+, \, \mbox{Im} \xi <0 $ and $\xi \mapsto\mathbf{D_p}(z,\xi)$ and $\xi \mapsto\mathbf{T_p}(z,\xi)$ are analytical on $\mathbb{C} \backslash  \rbrack - \infty, -\nuti_L \rbrack$. In the case where $\crit$, \eqref{C4:GaussDM} and \eqref{C4:GaussTM} are valid for $ z\in \mathbb{C} \backslash ( \, \rbrack - \infty, -\nuti_T \rbrack \cup \lbrack \nuti_L,+i\infty \lbrack \,), \,  z \notin \{\tilde{\nu}_L,\tilde{\nu}_T \}, \, \mbox{Im} z \geq 0$ and $\xi \mapsto\mathbf{D_p}(z,\xi)$ and $\xi \mapsto\mathbf{T_p}(z,\xi)$ are analytical on $\mathbb{C} \backslash ( \, \rbrack - \infty, -\nuti_T \rbrack \cup \lbrack \nuti_L,+i\infty \lbrack \,)$.