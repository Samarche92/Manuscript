...
In the case where $\crit$, $\partial \Omega_L$ is visble in Fig.~\ref{C4:domegaL}. During the deformation of contour $\Gamma_0$ into contour $\partial\Omega_L$, the pole $z$ is not crossed  and therefore does not contribute to the integral. However, if there exists $\zeta_{\xi} \in \Omega_L^+$ such that $T_L(\zeta_{\xi})=\xi$ and Im$\zeta_{\xi}<0$, then pole $\zeta_{\xi}$ is crossed during this deformation.
