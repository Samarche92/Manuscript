...
\begin{equation}
\begin{split}
T_*(\xi=\nuti\cos\theta)&=\xi \cos \varphi+\zeta_*(\xi)\sin \phiti \\
...
\label{C4:domega0}
\end{figure}
Note that in the case $\crit$, the pole $z$, Im$z\geq 0$ is not crossed during deformation from contour $\Gamma_0$ to contour $\partial \Omega_L$ and therefore does not contribute to the integral. However, the pole $\xi$ contributes in the case where there exists $\zeta_L$ such that $T_L(\zeta_L)=\xi$. To avoid this complication, in the case $\crit$, instead of deforming contour $\Gamma_0$ into $\partial \Omega_L$ for the L terms of operator $\mathbf{TM}$, contour $\Gamma_0$ is deformed into contour $\partial \Omega_T\cup\Gamma_1^b$, where $\partial \Omega_T$ is visble on Fig.~\ref{C4:domegaT} and $\Gamma_1^b$ is visible on Fig.~\ref{C4:gamma1cr}. The direction of the contour deformation is similar to the one shown on Fig.~\ref{C4:gamma1noncr}. Application of Cauchy's residue theorem now yields, for $V \in \mathbb{C}^3, \, z \notin \{\pm \tilde{\nu}_L,\pm \tilde{\nu}_T \}, z\in \mathbb{C} \backslash  (\rbrack - \infty, -\nuti_T \lbrack \cup i \lbrack \nuti_L , +\infty \lbrack )$ and Im$\xi <0 $ :
\begin{equation}
\int_{\Gamma_0} \mathbf{TM}(\xi,\zeta).\dfrac{V}{\zeta-z}\, d\zeta=\sum_{*=L,T} \dfrac{\mathbf{tm}_*(z).V}{\xi-T_*(z)} \mathbf{1}_{\Omega_T}(z)+\mathbf{T'_p}(z,\xi).V
\label{C4:GaussTMcr}
\end{equation}
where
\begin{equation}
T'_p(z,\xi)=\dfrac{1}{2i\pi}\left(\int_{\partial \Omega_T} D_T(\xi,\zeta).\dfrac{\mathbf{tm}_T(\zeta)}{\zeta-z}\,d\zeta+\int_{\partial \Omega_T\cup\Gamma_1^b} D_L(\xi,\zeta).\dfrac{\mathbf{tm}_L(\zeta)}{\zeta-z}\,d\zeta \right)
\end{equation}

It is important to not that during all of the aforementioned contour deformations, none of the branch points of the integrands are crossed. Therefore, it is assumed that Croisille and Lebeau's \cite{CroisilleLebeau} proof that $\mathbf{D_p}(z,\cdot)$ and $\mathbf{T_p}(z,\cdot)$ belong to a special class of functions $\mathcal{H}^3$ can be adapted to the 3D case. It will therefore be assumed that in the case $\noncr$, $\mathbf{D_p}(z,\cdot) \in \mathcal{H}^3$ and $\mathbf{T_p}(z,\cdot) \in \mathcal{H}^3$ and in the case $\crit$, $\mathbf{D_p}(z,\cdot) \in \tilde{\mathcal{H}}^3$ and $\mathbf{T_p}(z,\cdot) \in \mathcal{H}^3$. $\mathcal{H}$ and $\tilde{\mathcal{H}}$ are defined hereafter
\begin{definition}
\label{C4:defH}
