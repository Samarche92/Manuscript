...
\end{equation}
Using these definitions, contour $\Gamma_0$ in operator $\mathbf{TM}$ is deformed into contour $\partial \Omega_*^-$, visible on Fig.~\ref{C4:dOmegamoins}. The poles $\zeta$ of the integrand are $\zeta=T_*^{-1}(\xi)$. Let us now determine in which cases these poles are crossed by the contour deformation.
