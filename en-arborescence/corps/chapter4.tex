...
\label{C4:vjdef}
\end{equation}
Distributions $\alpha_j,\beta_j, \gamma_j $ are unknown and are belong to the special call $\mathcal{A}$ defined in chapter 2 of this thesis, Def.~\ref{defClassA}. We can now define the outgoing solution of $(\mathcal{P}^{\alpha})$ analogously to the 2D case :
\begin{definition}
...
The poles of $(\mathbf{M}-e^{-2i\varepsilon}\mathbf{\mathbb{I}_3})^{-1}$ are located in $\eta=\pm \zeta_*^{\varepsilon}(\xi)$, where $*=L,T$ and
\begin{equation}
...
\label{defnutieps}
\end{equation}
If the incident wave is longitudinal, then $\nuti_L^0, \nuti_T^0 \in \mathbb{R}$. However, if the incident wave is transverse, then two cases may occur :
\begin{itemize}
	\item if $|\sin\delta_{inc}| \leq \frac{\nu_L}{\nu_T}$, then $\nuti_L^0 \in \mathbb{R}$
	\item if $|\sin\delta_{inc}| >\frac{\nu_L}{\nu_T}$, then $\nuti_L=i\eta_L^0 \in i\mathbb{R}$ 
\end{itemize} 
In any case,
\begin{equation}
...
Integral \eqref{C4:vjeps} is well defined for all values of $\varepsilon \in \rbrack 0, \pi \lbrack $, since for these values of $\varepsilon$, the integration contour never crosses the branch points of the integrand, which are located at $\xi=\pm \tilde{\nu}_*^{\varepsilon}$, outside of the real axis.

According to Croisille et Lebeau \cite{CroisilleLebeau}, convergence in the 2D case is verified for $\varepsilon \rightarrow 0$. We will suppose that this is still the case in 3D. The integration contour $\mathbb{R}$ is deformed into contour $\Gamma_{0}$, visible on Fig.~\ref{C4:Gamma0_noncr} in the case where $\nuti_L\in\mathbb{R}$ and on Fig.~\ref{C4:Gamma0_cr} in the other case. In both cases, the branch points of the integrand are avoided.
...
\end{equation}
The far field parameter is $R=\frac{\omega r}{c_L}$. The aim is to determine the asymptotic behavior of $v(R\cos\theta\cos\delta_{\beta},R\sin\theta\cos\delta_{\beta})$ when $R\rightarrow +\infty$.The first step is to apply the following change of variables in integral $v_1$ :
\begin{equation}
\begin{split}
\xi&=\tilde{\nu}_*\cos\lambda \\
...
\draw[dashed,decoration={ markings,
      mark=at position 1.0 with {\arrow{>}}},
      postaction={decorate}] (0,-4) -- (0,3.65);
