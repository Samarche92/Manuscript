...
% 	\draw[ thick, ->] (2.5,-1.5) arc (0:-45:1);
% 	\node at (2.7,-1.9) {$F_1$};
% 	\draw[ thick, ->] (2.5,0.5) arc (0:45:1);
% 	\node at (2.7,0.9) {$F_2$};
	\draw[very thick, black,yshift=0pt,
	decoration={ markings,  % This schema allows for fine-tuning the positions of arrows 
		mark=at position 0.2 with {\arrow{latex}},
	...
	\end{tikzpicture}
	\caption{Integration contour $\Gamma_2$.}
	\label{C4:contour2}
\end{figure}

The first step of this procedure is to deform contour $\Gamma_0$ in \eqref{C4:regparteqn} into contour $\Gamma_2$, visible on Fig.~\ref{C4:contour2} for the case $\noncr$ and in Fig.~\ref{C4:contour2crit} for the case $\crit$.
